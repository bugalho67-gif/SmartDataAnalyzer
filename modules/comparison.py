...
def compare(df: pd.DataFrame):
    ...
    try:
        ...
        somente_arquivo1 = sorted(
            set(df.columns) - set(df2.columns)
        )

        somente_arquivo2 = sorted(
            set(df2.columns) - set(df.columns)
        )

        comuns = sorted(
            set(df.columns).intersection(df2.columns)
        )
        ...
        tipos = pd.DataFrame({
            "Arquivo 1": df.dtypes.astype(str),
            "Arquivo 2": df2.dtypes.astype(str)
        })
        ...
        st.error(
            f"Erro ao comparar os arquivos:\n\n{erro}"
        )
